...
def fixme(trace_dic, ab=True, xmax=None):
    ...
    if not isinstance(xmax, np.ndarray):
        xmax = np.max(np.abs(x), axis=0)  # /2.0
    # x = (x - xmax)/xmax
    xmax[xmax == 0] = 1
    x = x / xmax
    #y = y[ind]
...
def rescale_X(Xscaled, feat, X_range):  # X_range_path = path of the part1 file
    ...
    # pind = np.where(Xscaled[:, 0] > 0)
    # Xscaled[pind] = np.round(Xscaled[pind] * tp)
    # nind = np.where(Xscaled[:, 0] < 0)
    # Xscaled[nind] = np.round(Xscaled[nind] * -1 * tp)
...
class OurConvNet:
    ...
    def build(input_shape, classes):
        model = Sequential()
        # Block1
        model.add(Conv2D(32, kernel_size=(1, 8), strides=(1, 4), padding="same", input_shape=input_shape,
                         name='block1_conv1'))
        model.add(ELU(alpha=1.0, name='block1_act1'))
...
def DF_attack(x_adv, y_adv, feat_cell=10000, expanded=False, testsets=None, number_of_classes=83,
    ...
    history = model.fit(X_adv_tr, Y_adv_tr, batch_size=BATCH_SIZE, epochs=NB_EPOCH, verbose=VERBOSE,
...
def bhatResNet(num_classes, X_tr, Y_tr, X_te, Y_te):
    ...
    model.compile(loss='categorical_crossentropy',
                  ...
                  metrics=['accuracy', top1_acc, top2_acc, top3_acc, top4_acc, top5_acc, top6_acc, top7_acc, top8_acc,
                           top9_acc, top10_acc])
